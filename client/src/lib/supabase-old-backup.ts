...
/**
 * الحصول على طلبات المبيعات حسب الفترة
 */
export async function getSalesOrdersByPeriod(
  ...
  try {
    ...
    // تطبيق الفلتر حسب الفترة
    console.log('🔍 Filter params:', { period, date, month, year });
    if (period === 'daily' && date) {
      // فلتر يومي
      const startDate = `${date} 00:00:00`;
      const endDate = `${date} 23:59:59`;
      query = query.gte('date_order', startDate).lte('date_order', endDate);
    } else if (period === 'monthly' && month && year) {
      // فلتر شهري - استخدام تنسيق بسيط
      const startDate = `${year}-${String(month).padStart(2, '0')}-01`;
      const lastDay = new Date(year, month, 0).getDate();
      const endDate = `${year}-${String(month).padStart(2, '0')}-${lastDay}`;
      console.log('📅 Monthly filter:', { startDate, endDate, month, year });
      query = query.gte('date_order', startDate).lt('date_order', `${year}-${String(month + 1).padStart(2, '0')}-01`);
    } else if (period === 'yearly' && year) {
      // فلتر سنوي - استخدام تنسيق بسيط
      const startDate = `${year}-01-01`;
      const endDate = `${year + 1}-01-01`;
      query = query.gte('date_order', startDate).lt('date_order', endDate);
    } else {
      // الكل - جلب 1000 طلب فقط
    ...
    console.error('❌ Error fetching sales orders by period:', error);
    return [];
  }
}
